...
StoneParticle::StoneParticle(float x, float y, float xv, float yv) :
	x(x), y(y),
	xv(xv), yv(yv) {
...
void StoneParticle::updatePosition(float timeStep) {
...
void StoneParticle::bounceOffWalls(float damping) {
...
void StoneParticle::addDampingForce(float damping) {
	xf -= xv * damping;
	yf -= yv * damping;
}

void StoneParticle::draw() {
    
    glVertex2f(x, y);
}


void StoneParticle::displayParticle(){
//    ofNoFill();
    ofSetColor(col);
    ofDrawCircle(x, y, r);
...
//void StoneParticle::receiveGeometry(ofVec2f origin_, float externalRad_){
//    origin = origin_;
